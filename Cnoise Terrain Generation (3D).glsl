...
out vec4 EyeSpaceLightPosition;
out vec4 EyeSpacePosition;
out vec4 EyeSpaceObjectPosition;
...
{
  int levels = int ((e + 2) * 3);

  float amplitudeFactor = pow (2, f);
  float frequencyFactor = pow (2, g);
  ...
  for (int i = 0; i < levels; i++)
    {
       v = v + amplitude * cnoise (vec2 (frequency * x, frequency * y));
...
float getheight()
{
	  return clamp(cnoise(100, TimeElapsed), 1, 3);
}
...
void main(void)
{
  ...
  Position = ProjectionMatrix * ViewMatrix * WorldMatrix * Position;

  Normal = (ViewMatrix * WorldMatrix * vec4 (Normal, 0)).xyz;
